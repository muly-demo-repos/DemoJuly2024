/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { SensorWhereUniqueInput } from "../../sensor/base/SensorWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class SensorCreateNestedManyWithoutSensorTypesInput {
  @Field(() => [SensorWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SensorWhereUniqueInput],
  })
  connect?: Array<SensorWhereUniqueInput>;
}

export { SensorCreateNestedManyWithoutSensorTypesInput as SensorCreateNestedManyWithoutSensorTypesInput };
