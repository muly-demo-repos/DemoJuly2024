...
const service = {
  ...
  sensorReadings: () => FIND_MANY_RESULT,
  sensorReading: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("SensorReading", () => {
  ...
  test("POST /sensorReadings", async () => {
  ...
  test("GET /sensorReadings/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/sensorReadings"}/${nonExistingId}`)
  ...
  test("GET /sensorReadings/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/sensorReadings"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /sensorReadings existing resource", async () => {
